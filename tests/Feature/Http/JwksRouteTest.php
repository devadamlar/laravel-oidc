...
namespace DevAdamlar\LaravelOidc\Tests\Feature\Http;
...
class JwksRouteTest extends TestCase
{
    ...
    public function test_should_return_no_jwks_if_no_private_key_provided(): void
    {
        // Arrange
        Log::shouldReceive('warning')->with('JWKS URI is set, but no private key found.')->once();
        config()->set('oidc.rp_signing_algorithm', 'ES256');
    ...
    public function test_should_return_jwks_based_on_given_private_keys_in_global_config(): void
    {
        // Arrange
        config()->set('oidc.cache_driver', 'array');
        config()->set('oidc.cache_ttl', 86400);
        config()->set('oidc.private_key', 'certs/private.pem');
        config()->set('oidc.rp_signing_algorithm', 'ES256');
        config()->set('oidc.signing_algorithm', 'RS256');
        config()->set('auth.guards', [
            'session' => ['driver' => 'session', 'name' => 'id'],
            'jwt' => ['driver' => 'jwt', 'key' => 'key'],
...
PEM
    ...
    public function test_should_return_jwks_based_on_given_private_keys_in_guard_configs_only(): void
    {
        // Arrange
        config()->set('oidc.cache_driver', 'array');
        config()->set('oidc.cache_ttl', 86400);
